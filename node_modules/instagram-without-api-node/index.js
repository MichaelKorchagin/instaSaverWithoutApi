        ...
        return new Promise(function (resolve) {
            ...
            request(options, async (error, res, body) => {
                ...
                if (!error && res.statusCode == 200) {
                    ...
                    const pattern = new RegExp(/"media_id":"(\w+)\"/);
                    const realId = res.toJSON().body.match(pattern)[1];
                    ...
                    const image =
                            json?.['items']?.[0]?.['image_versions2']?.['candidates']?.[0]?.['url'] ?
                                ...
                                json?.['items']?.[0]?.['carousel_media']?.[0]?.['image_versions2']?.['candidates']?.[0]?.['url'] ?
                                    ...
                                    false;
                    if (json?.['items']?.[0]?.['video_versions'].length > 0) {
    ...
    return new Promise(function (resolve) {
        ...
        try {
            fs.stat(file, async (err, stats) => {
                ...
                if (unixTimestampNow - unixTimestamp >= 0) result = await callback(options)
